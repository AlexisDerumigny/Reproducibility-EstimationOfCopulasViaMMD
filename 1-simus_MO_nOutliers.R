...
vecNameEstimators = c("MLE", "Itau",
                      "MMD_gaussian.Phi", "MMD_gaussian")
vecGamma = c(gaussian = 0.2, gaussian.Phi = 0.8)


# Main loop ==========================================================
pb = pbapply::startpb(min = 0, max = NReplications * length(nOutliers))
i_pb = 1
...
for (iRep_ in 1:NReplications)
{
  for (i_outliers in 1:length(nOutliers))
  {
    ...
    resultMMD_gaussian1 <- BiCopEst.MO(u1 = U[,1], u2 = U[,2], method = "MMD",
                                       kernel = nameKernel1, gamma = vecGamma["gaussian"],
    ...
    # MMD with Phi
    time1 = proc.time()
    nameKernel2 = paste0(nameKernel1, ".Phi")
    resultMMD_gaussian2 <- BiCopEst.MO(u1 = U[,1], u2 = U[,2], method = "MMD",
                                       kernel = nameKernel2, gamma = vecGamma["gaussian.Phi"],
                                       par.start = par.start, alpha = alpha_estimation,
                                       niter = niter, ndrawings = ndrawings,
                                       naveraging = naveraging
    )
    ...
    toWrite3 = c(vec_params, vecGamma["gaussian"], paste("MMD", nameKernel1 , sep = '_'),
                 resultMMD_gaussian1$par, as.numeric(timeMMD_gaussian1))

    toWrite4 = c(vec_params, vecGamma["gaussian.Phi"], paste("MMD", nameKernel2 , sep = '_'),
