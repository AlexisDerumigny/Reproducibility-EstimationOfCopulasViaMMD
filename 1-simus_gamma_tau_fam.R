...
NReplications = 200
nSample = 1000
vecPossibleTaus = seq(0.1, 0.9, by = 0.1)
...
listMethodsTauInit = c("unif(0.05;0.95)", "empKT")
nameKernel = "gaussian"


# Main loop ==========================================================
pb = pbapply::startpb(
  min = 0, max = 3 * 2 * length(vecPossibleTaus) * NReplications *
    length(vecPossibleGammas)  * length(listMethodsTauInit))
    ...
    for (trueTau in vecPossibleTaus)
    {
      for (iRep_ in 1:NReplications)
      {
        ...
        for (i_gamma in 1:length(vecPossibleGammas))
        {
          ...
          for (methodTauInit in listMethodsTauInit)
          {
            tauInit = switch (
              methodTauInit,
